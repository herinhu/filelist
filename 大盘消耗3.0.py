...
#%%几个关注的渠道
follow_list=[10108,10010,10167,10097,10008]
...
for i in cnt_foll['name'].drop_duplicates():
       line.add(i,cnt_foll[cnt_foll['name']==i]['date'].apply(lambda x:str(x)),cnt_foll[cnt_foll['name']==i]['cost_r'].apply(lambda x:int(x)),legend_pos="10%") 



#TOP 7 渠道消耗
top_cnt=df.groupby(by=['channel_id','date'],as_index=False).sum()[['channel_id','date','cost_r']]
cnt=top_cnt[top_cnt['date'].apply(lambda x:str(x))==end_time.replace('-', '')].sort_values(by='cost_r').tail(7)['channel_id']
...
for i in top_cnt['name'].drop_duplicates():
       line1.add(i,top_cnt[top_cnt['name']==i]['date'].apply(lambda x:str(x)),top_cnt[top_cnt['name']==i]['cost_r'].apply(lambda x:int(x)),legend_pos="60%") 
...
def spot_line(cnt,name):
    ...
    for i in top_spt['name'].drop_duplicates():
           line.add(i,top_spt[top_spt['name']==i]['date'].apply(lambda x:str(x)),top_spt[top_spt['name']==i]['cost_r'].apply(lambda x:int(x)),legend_pos="75%",mark_point=['max']) 
    grid= Grid(width=1000,height=350)
    grid.add(line,grid_right="25%")
    page.add_chart(grid)
    
    '''                     涨跌幅TOP广告位                '''
    if follow_spot['spot_id'].drop_duplicates().count()<=7:
        x=1   
    else:
        lists=[]
        ...
        line = Line(name+'涨幅TOP广告位')
        for i in rise_spt['name'].drop_duplicates():
               line.add(i,rise_spt[rise_spt['name']==i]['date'].apply(lambda x:str(x)),rise_spt[rise_spt['name']==i]['cost_r'].apply(lambda x:int(x)),legend_orient='vertical',legend_pos="45%",is_legend_show=False) 
        ...
        line1 = Line(name+'跌幅TOP广告位',title_pos="50%")
        for i in fall_spt['name'].drop_duplicates():
               line1.add(i,fall_spt[fall_spt['name']==i]['date'].apply(lambda x:str(x)),fall_spt[fall_spt['name']==i]['cost_r'].apply(lambda x:int(x)),legend_pos="87%") 
        #画图
        grid= Grid(width=1200,height=350)
        grid.add(line,grid_right="55%")
        grid.add(line1,grid_left="55%")
...
def user_table(cnt,name):
    ...
    fall_user['涨幅TOP客户']=fall_user['user_id'].map(str)+'—'+fall_user['username'].map(str)
    fall_user=fall_user[['date','涨幅TOP客户','cost_r']]
    fall_user.columns=['日期','涨幅TOP客户','原始消耗']
    fall_user=fall_user.pivot('涨幅TOP客户','日期','原始消耗').applymap(lambda x:int(x))
